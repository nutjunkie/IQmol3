...
# https://gitlab.kitware.com/cmake/cmake/-/commit/dea324f8041753bceec6a09fc6420569ef6817a1
# with patches for https://gitlab.kitware.com/cmake/cmake/-/issues/22959 and
# https://gitlab.kitware.com/cmake/cmake/-/issues/18872.
...
The following targets will be defined depending on which components were enabled and detected:
...
``HDF5_FIND_QUIETLY``
  ...
  Set ``true`` to silence more output, primarily from calling the compiler wrapper.
#]=======================================================================]
...
# Check if any file's name (no directory components) in the provided `lst`
# contains the given glob.
function(_file_name_contains lst glob contains)
    set(${contains} 0 PARENT_SCOPE)
    foreach(item ${${lst}})
        get_filename_component(_name "${item}" NAME)
        if("${_name}" MATCHES ".*${glob}.*")
...
if(NOT HDF5_FIND_COMPONENTS)
  ...
  _remove_duplicates(HDF5_LANGUAGE_BINDINGS)
endif()
...
# Figure out if the discovered library was actually static or shared.
function(_determine_library_type library_type main_library)
  if(APPLE)
    if(${main_library} MATCHES "\\.dylib\\.?")
      set(_library_type "SHARED")
    elseif(${main_library} MATCHES "\\.a$")
      set(_library_type "STATIC")
    endif()
  elseif(CYGWIN)
    if(${main_library} MATCHES "\\.dll.a$")
      set(_library_type "SHARED")
    elseif(${main_library} MATCHES "\\.a$")
      set(_library_type "STATIC")
    endif()
  elseif(UNIX)
    if(${main_library} MATCHES "\\.so\\.?")
      set(_library_type "SHARED")
    elseif(${main_library} MATCHES "\\.a$")
      set(_library_type "STATIC")
    endif()
  elseif(WIN32)
    ...
    message(WARNING "Unknown platform for determining HDF5 library type of ${main_library}")
  endif()
  set(${library_type} ${_library_type} PARENT_SCOPE)
endfunction(_determine_library_type)
...
function( _HDF5_invoke_compiler language output_var return_value_var version_var is_parallel_var)
  ...
  if(HDF5_FIND_DEBUG)
    message(STATUS "HDF5 ${language} compiler execute output: ${output}")
  ...
  if(return_value AND NOT HDF5_FIND_QUIETLY)
    ...
    if(HDF5_FIND_DEBUG)
      message(STATUS "HDF5 ${language} compiler show output: ${output}")
    endif(HDF5_FIND_DEBUG)
    if(return_value AND NOT HDF5_FIND_QUIETLY)
      message(STATUS
    ...
    if(HDF5_FIND_DEBUG)
      message(STATUS "HDF5 ${language} compiler showconfig output: ${config_output}")
    endif(HDF5_FIND_DEBUG)
    if(return_value AND NOT HDF5_FIND_QUIETLY)
      message(STATUS
    ...
    if( HDF5_FOUND)
        ...
        foreach(_lang ${HDF5_LANGUAGE_BINDINGS})
            ...
            if( _hdf5_lang_location )
                ...
                _determine_library_type(HDF5_${_lang}_LIBRARY_TYPE HDF5_${_lang}_LIBRARY)
                if(WIN32 AND HDF5_${_lang}_LIBRARY_TYPE STREQUAL "SHARED")
                  list(APPEND HDF5_${_lang}_DEFINITIONS "-DH5_BUILT_AS_DYNAMIC_LIB")
                endif()
            ...
            if(HDF5_FIND_HL)
                ...
                if( _hdf5_lang_hl_location )
                    ...
                    _determine_library_type(HDF5_${_lang}_HL_LIBRARY_TYPE HDF5_${_lang}_HL_LIBRARY)
                    if(WIN32 AND HDF5_${_lang}_HL_LIBRARY_TYPE STREQUAL "SHARED")
                      list(APPEND HDF5_${_lang}_HL_DEFINITIONS "-DH5_BUILT_AS_DYNAMIC_LIB")
                    endif()
      ...
      if(HDF5_${_lang}_COMPILER_EXECUTABLE)
        ...
        if(HDF5_${_lang}_RETURN_VALUE EQUAL 0)
          if(HDF5_FIND_DEBUG)
            ...
            message(STATUS "HDF5_${_lang}_COMPILER_EXECUTABLE: ${HDF5_${_lang}_COMPILER_EXECUTABLE}")
          endif()
          ...
          set(HDF5_${_lang}_LIBRARIES)
          if(HDF5_FIND_DEBUG)
            message(STATUS "Output from parsing ${_lang} compile line:
              HDF5_${_lang}_COMPILE_LINE: ${HDF5_${_lang}_COMPILE_LINE}
              HDF5_${_lang}_INCLUDE_DIRS: ${HDF5_${_lang}_INCLUDE_DIRS}
              HDF5_${_lang}_DEFINITIONS: ${HDF5_${_lang}_DEFINITIONS}
              HDF5_${_lang}_LIBRARY_DIRS: ${HDF5_${_lang}_LIBRARY_DIRS}
              HDF5_${_lang}_LIBRARY_NAMES: ${HDF5_${_lang}_LIBRARY_NAMES}
              HDF5_${_lang}_HL_LIBRARY_NAMES: ${HDF5_${_lang}_HL_LIBRARY_NAMES}")
          endif(HDF5_FIND_DEBUG)
          ...
          foreach(_lib IN LISTS HDF5_${_lang}_LIBRARY_NAMES)
            ...
            if(HDF5_${_lang}_LIBRARY_${_lib})
              if("x${_lib}" MATCHES "hdf5")
                _determine_library_type(HDF5_${_lang}_LIBRARY_TYPE HDF5_${_lang}_LIBRARY_${_lib})
                if(HDF5_FIND_DEBUG)
                  message(STATUS "HDF5_${_lang}_LIBRARY_TYPE: ${HDF5_${_lang}_LIBRARY_TYPE}")
                endif(HDF5_FIND_DEBUG)
                # TODO something similar for HL
              endif()
          ...
          set(HDF5_${_lang}_FOUND TRUE)
          _remove_duplicates(HDF5_${_lang}_DEFINITIONS)
          _remove_duplicates(HDF5_${_lang}_INCLUDE_DIRS)
        else()
    ...
    if(HDF5_${_lang}_FOUND)
      if(NOT HDF5_${_lang}_COMPILER_NO_INTERROGATE)
        ...
        if(HDF5_FIND_HL)
          ...
          list(APPEND HDF5_HL_DEFINITIONS ${HDF5_${_lang}_HL_DEFINITIONS})
        endif()
      endif()
    endif()
  endforeach()
  _remove_duplicates(HDF5_DEFINITIONS)
  _remove_duplicates(HDF5_INCLUDE_DIRS)
  set(HDF5_FOUND TRUE)
  set(HDF5_REQUIRED_VARS HDF5_LIBRARIES)
  if(HDF5_FIND_HL)
    list(APPEND HDF5_REQUIRED_VARS HDF5_HL_LIBRARIES)
    _remove_duplicates(HDF5_HL_DEFINITIONS)
  endif()
endif()
    ...
    foreach(_lang IN LISTS HDF5_LANGUAGE_BINDINGS)
        ...
        foreach(LIB IN LISTS HDF5_${_lang}_LIBRARY_NAMES)
            if(HDF5_USE_STATIC_LIBRARIES)
                ...
                # TODO this needs to be set properly for if a shared library
                # was found even when a static library was requested
                if(WIN32)
                  list(APPEND HDF5_${_lang}_DEFINITIONS "-DH5_BUILT_AS_DYNAMIC_LIB")
                endif()
            endif()
            ...
            _determine_library_type(HDF5_${_lang}_LIBRARY_TYPE _hdf5_main_library)
            if(HDF5_FIND_DEBUG)
              message(STATUS "HDF5_${_lang}_LIBRARY_TYPE: ${HDF5_${_lang}_LIBRARY_TYPE}")
            endif(HDF5_FIND_DEBUG)
        ...
        if(HDF5_FIND_HL)
            foreach(LIB IN LISTS HDF5_${_lang}_HL_LIBRARY_NAMES)
                if(HDF5_USE_STATIC_LIBRARIES)
                    ...
                    set( THIS_LIBRARY_SEARCH_DEBUG ${LIB}d ${LIB}_D ${LIB}_debug ${LIB}d-shared ${LIB}_D-shared ${LIB}_debug-shared)
                    set( THIS_LIBRARY_SEARCH_RELEASE ${LIB} ${LIB}-shared)
                    # TODO this needs to be set properly for if a shared
                    # library was found even when a static library was
                    # requested
                    if(WIN32)
                      list(APPEND HDF5_${_lang}_HL_DEFINITIONS "-DH5_BUILT_AS_DYNAMIC_LIB")
                    endif()
                ...
                # Set the "main" library if not already set.
                if (NOT _hdf5_hl_library)
                  if (HDF5_${LIB}_LIBRARY_RELEASE)
                    set(_hdf5_hl_library "${HDF5_${LIB}_LIBRARY_RELEASE}")
                  elseif (HDF5_${LIB}_LIBRARY_DEBUG)
                    set(_hdf5_hl_library "${HDF5_${LIB}_LIBRARY_DEBUG}")
                  endif ()
                endif ()
                set(HDF5_${LIB}_HL_LIBRARY ${_hdf5_hl_library})
                unset(_hdf5_hl_library)

                _determine_library_type(HDF5_${LIB}_HL_LIBRARY_TYPE HDF5_${LIB}_HL_LIBRARY)
                if(HDF5_FIND_DEBUG)
                  message(STATUS "HDF5_${LIB}_HL_LIBRARY_TYPE: ${HDF5_${LIB}_HL_LIBRARY_TYPE}")
                endif(HDF5_FIND_DEBUG)

                select_library_configurations( HDF5_${LIB} )
                list(APPEND HDF5_${_lang}_HL_LIBRARIES ${HDF5_${LIB}_HL_LIBRARY})
            endforeach()
...
macro(_add_hdf5_dependency _additional_lib prefer_static)
  if(WIN32)
    set(_libnames lib${_additional_lib}.lib)
  else(WIN32)
    if(${prefer_static})
      set(_libnames lib${_additional_lib}.a ${_additional_lib})
    else(${prefer_static})
  ...
  if(${${_libvar}} STREQUAL "${_libvar}-NOTFOUND")
    ...
    if(HDF5_FIND_DEBUG)
      message(STATUS "Found ${_additional_lib} (${_libvar}): ${${_libvar}}")
    endif(HDF5_FIND_DEBUG)
    list(APPEND HDF5_LIBRARIES ${${_libvar}})
    foreach(_lang IN LISTS HDF5_LANGUAGE_BINDINGS)
      list(APPEND HDF5_${_lang}_LIBRARIES ${${_libvar}})
      _remove_duplicates(HDF5_${_lang}_LIBRARIES)
      if(HDF5_FIND_HL)
        list(APPEND HDF5_${_lang}_HL_LIBRARIES ${${_libvar}})
        _remove_duplicates(HDF5_${_lang}_HL_LIBRARIES)
      endif(HDF5_FIND_HL)
    endforeach(_lang IN LISTS HDF5_LANGUAGE_BINDINGS)
  endif()
  _remove_duplicates(HDF5_LIBRARIES)
endmacro(_add_hdf5_dependency)

if(HDF5_FOUND)
  # Since there may not be a compiler line to help us, add the additional
  # required libraries manually.
  #
  # TODO this also needs a compiler line-based implementation or something.
  if(HDF5_USE_STATIC_LIBRARIES)
    set(_prefer_static 1)
  else(HDF5_USE_STATIC_LIBRARIES)
    set(_prefer_static 0)
  endif(HDF5_USE_STATIC_LIBRARIES)
  _add_hdf5_dependency(sz ${_prefer_static})
  _add_hdf5_dependency(aec ${_prefer_static})
  if(WIN32)
    _add_hdf5_dependency(zlib 0)
  else(WIN32)
    _add_hdf5_dependency(z ${_prefer_static})
    _add_hdf5_dependency(m 0)
    _add_hdf5_dependency(rt 0)
    _add_hdf5_dependency(dl 0)
  endif(WIN32)
endif(HDF5_FOUND)
  ...
  foreach (hdf5_lang IN LISTS HDF5_LANGUAGE_BINDINGS)
    ...
    if (NOT TARGET "hdf5::${hdf5_target_name}")
      ...
      if (HDF5_COMPILER_NO_INTERROGATE)
        # TODO something other than INTERFACE lib?
        # TODO why no INTERFACE_LINK_LIBRARIES?
        ...
        # TODO sorry for the hack
        set(_hdf5_libs "${HDF5_${hdf5_lang}_LIBRARIES}")
        set(_hdf5_libtype "${HDF5_${hdf5_lang}_LIBRARY_TYPE}")
        ...
        # TODO why did this only use IMPORTED_IMPLIB and not
        # INTERFACE_LINK_LIBRARIES?
        set_target_properties("hdf5::${hdf5_target_name}" PROPERTIES
          IMPORTED_LOCATION "${_hdf5_location}"
          IMPORTED_IMPLIB "${_hdf5_location}"
          INTERFACE_LINK_LIBRARIES "${_hdf5_libs}"
          INTERFACE_INCLUDE_DIRECTORIES "${HDF5_${hdf5_lang}_INCLUDE_DIRS}"
          INTERFACE_COMPILE_DEFINITIONS "${_hdf5_definitions}")
        unset(_hdf5_libtype)
    ...
    if(HDF5_FIND_DEBUG)
      get_target_property(HDF5_${hdf5_lang}_LIB_TYPE "hdf5::${hdf5_target_name}" TYPE)
      message(STATUS "HDF5_${hdf5_lang}_LIB_TYPE: ${HDF5_${hdf5_lang}_LIB_TYPE}")
    endif(HDF5_FIND_DEBUG)
    ...
    if (NOT TARGET "hdf5::${hdf5_target_name}")
      ...
      if (HDF5_COMPILER_NO_INTERROGATE)
        ...
        # TODO why no INTERFACE_LINK_LIBRARIES?
        set_target_properties("hdf5::${hdf5_target_name}" PROPERTIES
          INTERFACE_LINK_LIBRARIES "${HDF5_${hdf5_lang}_HL_LIBRARIES}"
          INTERFACE_INCLUDE_DIRECTORIES "${HDF5_${hdf5_lang}_HL_INCLUDE_DIRS}"
          INTERFACE_COMPILE_DEFINITIONS "${_hdf5_definitions}")
      else()
        # TODO why aren't all of these HL?
        if (DEFINED "HDF5_${hdf5_target_name}_LIBRARY")
          set(_hdf5_location "${HDF5_${hdf5_target_name}_LIBRARY}")
        ...
        # TODO sorry for the hack
        set(_hdf5_libs "${HDF5_${hdf5_lang}_HL_LIBRARIES}")
        # TODO should this be HL specific?
        set(_hdf5_libtype "${HDF5_${hdf5_lang}_LIBRARY_TYPE}")
        add_library("hdf5::${hdf5_target_name}" ${_hdf5_libtype} IMPORTED)
        # TODO why did this only use IMPORTED_IMPLIB and not
        # INTERFACE_LINK_LIBRARIES?
...
if (HDF5_FIND_DEBUG)
  ...
  foreach(_lang IN LISTS HDF5_LANGUAGE_BINDINGS)
    ...
    message(STATUS "HDF5_${_lang}_HL_DEFINITIONS: ${HDF5_${_lang}_HL_DEFINITIONS}")
