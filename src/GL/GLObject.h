...
namespace GL {
   ...
   /// Abstract base class for any rigid body item that can be draw on the GL
   /// canvas.  / Derived classes must implement a draw() function and, where the
   /// appearance depends on the camera orientation, draw(cameraPosition).
   /// An example of this is when double Bonds are drawn (see BondLayer.h).
   ///
