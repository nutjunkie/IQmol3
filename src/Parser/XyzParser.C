...
{
   const QRegularExpression integerOnly("^\\d+$");  // note nextLine() returns a trimmed line
   QRegularExpression anyReal("([-+]?[0-9]*\\.[0-9]+([eE][-+]?[0-9]+)?)");
   ...
   if (ok) {
      ...
      QRegularExpressionMatch match(anyReal.match(line));
      if (match.hasMatch()) energy = match.captured(1).toDouble(&ok);
