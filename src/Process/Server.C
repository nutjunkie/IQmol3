...
void Server::submit(Job* job)
{
   ...
   if (isWebBased()) {
      if(job->get<QString>("jobType")=="gromacs"){
         ...
         Network::HttpConnection* conn = dynamic_cast<Network::HttpConnection*>(m_connection);
         Network::Reply* reply(conn->postJsonFiles(fileName,payload,submit));
   ...
   switch (m_configuration.queueSystem()) {
      ...
      case ServerConfiguration::AWS: {
         if (m_connection) {
            ...
            status = JobInfo::fromString(map["status"]);
