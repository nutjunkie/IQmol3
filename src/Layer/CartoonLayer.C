...
Cartoon::Cartoon(Data::Surface& surface) : m_surface(surface), m_configurator(*this), 
   m_drawMode(Fill), m_callListPositive(0), m_drawVertexNormals(false),
   m_drawFaceNormals(false), m_balanceScale(false), m_molecule(0), m_decimator(0)
{
...
void Cartoon::setMolecule(Molecule* molecule) 
{
   m_molecule = molecule;
...
GLuint Cartoon::compile(Data::Mesh const& mesh)
{
...
void Cartoon::clearPropertyData()
{
...
void Cartoon::computePropertyData(Function3D const& function) 
{
...
void Cartoon::computeIndexField() 
{
...
void Cartoon::dumpMeshInfo() const
{
   m_surface.dump();
}
