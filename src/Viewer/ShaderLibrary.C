...
ShaderLibrary::ShaderLibrary(QOpenGLFunctions* functions) :
   m_glFunctions(functions),
   m_normalBuffer(0), 
   m_filterBuffer(), 
   m_rotationTextureId(0), 
   m_rotationTextureSize(64), 
   m_rotationTextureData(0),
   m_filtersAvailable(false), 
...
// This should be moved into the Viewer class
...
void ShaderLibrary::resizeScreenBuffers(QSize const& windowSize, double* projectionMatrix)
{
   ...
   m_normalBuffer = new QOpenGLFramebufferObject(windowSize, QOpenGLFramebufferObject::Depth);
   m_filterBuffer = new QOpenGLFramebufferObject(windowSize, QOpenGLFramebufferObject::Depth);
