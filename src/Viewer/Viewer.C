...
void Viewer::editShaders()
{
   ...
   if (!m_shaderDialog) {
      ...
      connect(m_shaderDialog, SIGNAL(updated()), this, SLOT(update()));
...
void Viewer::editCamera()
{
   if (!m_cameraDialog) {
      ...
      connect(m_cameraDialog, SIGNAL(updated()), this, SLOT(update()));
...
void Viewer::qglColor(QColor const& color) 
{
   glColor4f(color.redF(), color.greenF(), color.blueF(), color.alphaF());
}


void Viewer::qglClearColor(QColor const& clearColor) 
{
...
/*
void Viewer::renderText(double const x, double const y, double const z, 
   const QString& str, QFont const& font) 
{
...
void Viewer::drawLabels(GLObjectList const& objects)
{
   ...
   if (!msg.isEmpty()) {
      ...
      drawText(width()-s_labelFontMetrics.horizontalAdvance(msg), height()-10, msg);
   }
...
int count(0);
for (count = 0; count < m_objects.size(); ++count) {
...
void Viewer::drawSelectionRectangle(QRect const& rect) {
